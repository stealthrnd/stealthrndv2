  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const job = {
        ...
        // Jobs born here (office.html) are owned by this store from the start.
        // migrate.js only sets 'trello' when it seeds a card it imported.
        stageSource: body.stageSource ?? 'jobs',
        dadJob: !!body.dadJob,
    ...
    if (req.method === 'PATCH') {
      ...
      // Once the stage is set from here, this store owns it. migrate.js reads
      // this flag and stops re-seeding stage from the Trello list, so a later
      // migrate re-run can't revert a move made on the tablet.
      if ('stage' in body && body.stage !== jobs[i].stage) {
        body.stageSource = 'jobs';
      }
      jobs[i] = { ...jobs[i], ...body };
      await writeJobs(jobs);
